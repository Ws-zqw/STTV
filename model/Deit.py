...
class hMLP_stem(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=16, in_chans=3, embed_dim=768, norm_layer=nn.SyncBatchNorm):
        super().__init__()
...
def deit_small_patch16_LS(pretrained=True, img_size=(224, 224), pretrained_21k=True, num_classes=1000, **kwargs):
    model = vit_models(
        img_size=img_size, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=Layer_scale_init_Block, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        # name = 'https://dl.fbaipublicfiles.com/deit/deit_3_small_' + str(img_size[0]) + '_'
        name = 'https://dl.fbaipublicfiles.com/deit/deit_3_small_384_'
        if pretrained_21k:
            name += '21k.pth'
        else:
            name += '1k.pth'
