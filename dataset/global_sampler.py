...
class DistributedMiningSampler(DistributedSampler[T_co]):
    ...
    def generate_indices_sim(self):
        ...
        # else:
            ...
            #     gps_list = []
            #     for j in range(index_s, index_e):
            #         idx = random.choice(self.dataset.neighborhood_id[ori_list[j]])
            #         # random sampling until no overlap in the batch
            #         while idx in ori_list[index_s:index_e] or idx in gps_list:
            #             idx = random.choice(self.dataset.neighborhood_id[ori_list[j]])
            #         gps_list.append(idx)
            #     self.current_indices.extend(gps_list)
